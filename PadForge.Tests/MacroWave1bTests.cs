...
namespace PadForge.Tests
{
    ...
    public class MacroWave1bTests
    {
        ...
        public void Wave1b_ActionTypes_AppendedAtEnumTail_WithPinnedOrdinals()
        {
            ...
            Assert.Equal(MacroActionType.VoiceListenWhileHeld, values[^2]);
            Assert.Equal(54, (int)MacroActionType.VoiceListenWhileHeld);
            Assert.Equal(MacroActionType.HeadphoneVolumeDown, values[^3]);
            Assert.Equal(MacroActionType.HeadphoneVolumeUp, values[^4]);
            ...
            Assert.Equal(MacroActionType.AxisLatchRelease, values[^6]);
            Assert.Equal(MacroActionType.AxisSetLatched, values[^7]);
            ...
            Assert.Equal(MacroActionType.ComboBreak, values[^8]);
            Assert.Equal(MacroActionType.AxisAdd, values[^9]);
            Assert.Equal(MacroActionType.ToggleWheel, values[^10]);
            Assert.Equal(MacroActionType.RepeatVcAxisWhileHeld, values[^11]);
            Assert.Equal(MacroActionType.ToggleVcAxis, values[^12]);
            Assert.Equal(MacroActionType.ToggleMouseButton, values[^13]);
            Assert.Equal(MacroActionType.CycleTapList, values[^14]);
            Assert.Equal(MacroActionType.MouseNudge, values[^15]);
            Assert.Equal(MacroActionType.MouseWheelTap, values[^16]);
            Assert.Equal(MacroActionType.AxisHold, values[^17]);
